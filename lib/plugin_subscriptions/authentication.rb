...
class PluginSubscriptions::Authentication
  ...
  def get_keys(request_id)
    raw = PluginStore.get(PluginSubscriptions::Subscriptions.namespace, "#{keys_db_key}_#{request_id}")
    OpenStruct.new(
  ...
  def set_keys(request_id, user_id, rsa, nonce)
    PluginStore.set(PluginSubscriptions::Subscriptions.namespace, "#{keys_db_key}_#{request_id}",
      user_id: user_id,
      pem: rsa.export,
      nonce: nonce
    )
  ...
  def delete_keys(request_id)
    PluginStore.remove(PluginSubscriptions::Subscriptions.namespace, "#{keys_db_key}_#{request_id}")
  end

  def get_client_id
    PluginStore.get(PluginSubscriptions::Subscriptions.namespace, client_id_db_key)
  end

  def set_client_id
    client_id = SecureRandom.hex(32)
    PluginStore.set(PluginSubscriptions::Subscriptions.namespace, client_id_db_key, client_id)
    client_id
  end
end
