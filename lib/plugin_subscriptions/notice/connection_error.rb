...
class PluginSubscriptions::Notice::ConnectionError
  ...
  def create!
    if attrs = current_error
      ...
      domain = PluginSubscriptions::Notice.send("#{archetype.to_s}_domain")
  ...
  def namespace
    "#{PluginSubscriptions::PLUGIN_NAME}_notice_connection"
  end

  def current_error(query_only: false)
    @current_error ||= begin
      query = PluginStoreRow.where(plugin_name: namespace)
      query = query.where("(value::json->>'archetype')::integer = ?", PluginSubscriptions::Notice.archetypes[archetype.to_sym])
