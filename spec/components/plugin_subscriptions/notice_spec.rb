...
describe PluginSubscriptions::Notice do
  ...
  context "plugin status" do
    ...
    it "converts warning into notice" do
      ...
      expect(notice.message).to eq(I18n.t("admin.plugin_subscriptions.notice.compatibility_issue.message", domain: described_class.plugin_status_domain))
  ...
  context "connection errors" do
    ...
    it "creates an error if connection to notice server fails" do
      ...
      error = PluginSubscriptions::Notice::ConnectionError.new(:plugin_status)
    ...
    it "only creates one connection error per type at a time" do
      ...
      plugin_status_errors = PluginSubscriptions::Notice::ConnectionError.new(:plugin_status)
      subscription_message_errors = PluginSubscriptions::Notice::ConnectionError.new(:subscription_message)
